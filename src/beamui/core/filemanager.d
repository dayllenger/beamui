    ...
    try
    {
        version (Windows)
        {
            ...
            auto res = CreateProcessW(null, //explorerPath.toUTF16z,
                    cast(wchar*)arg.toUTF16z, null, null, false,
        ...
        else static if (isFreedesktop)
        {
                    ...
                    string iconName = null, string displayName = null, string fileName = null) pure
            {
                ...
                foreach (token; unquotedArgs)
                {
                    ...
                    {
                        static void expand(string token, ref string expanded, ref size_t restPos,
                                ref size_t i, string insert)
                        {
                            if (token.length == 2)
                            {
                                expanded = insert;
            ...
            foreach (mimeinfoCache; allAppDirs.map!(p => buildPath(p, "mimeinfo.cache")))
            {
                try
                {
                    parseConfigFile(mimeinfoCache, "MIME Cache", delegate bool(string key, string value) {
                        ...
                        { //no need to proceed, since MIME types are sorted in alphabetical order.
                            return false;
                        }
                        if (key.equal("inode/directory") && value.length)
                        {
