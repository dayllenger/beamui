...
*/
module beamui.graphics.iconprovider;
...
static if (BACKEND_GUI)
{
    version (Windows)
    {
        ...
        private extern (Windows) HRESULT _dummy_SHGetStockIconInfo(SHSTOCKICONID siid, UINT uFlags,
        ...
        class WindowsIconProvider : IconProviderBase
        {
            this()
            {
                ...
                _SHGetStockIconInfo = cast(typeof(&_dummy_SHGetStockIconInfo))wenforce(GetProcAddress(_shell,
                        "SHGetStockIconInfo"), "Could not load SHGetStockIconInfo");
            }
        ...
        alias NativeIconProvider = WindowsIconProvider;
    }
    else static if (isFreedesktop)
    {
        ...
        class FreedesktopIconProvider : IconProviderBase
        {
            ...
            DrawBufRef getIconFromTheme(string name, string context = null)
            {
                ...
                try
                {
                    if (context.length)
                    {
                        iconPath = findClosestIcon!(subdir => subdir.context == context)(name, 32,
                                _iconThemes, _baseIconDirs);
                    }
                    else
                    {
        ...
        alias NativeIconProvider = FreedesktopIconProvider;
    }
    else
    {
